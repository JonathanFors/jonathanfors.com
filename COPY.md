# Site copy — `/redesign` homepage + 404

Every word on the page, in the order it appears. **Edit this file freely** — change
the text in the right-hand column / quoted blocks, then tell me and I'll apply it to
the code.

Sections 0–10 are the redesigned homepage at `/redesign`. Section 11 is the 404
page, which is live site-wide already — it's what any wrong URL shows.

**How to read this**
- `▸ Field` — a label naming the slot, so we both know which bit we mean.
- Text in **bold** inside a line is set in the red "tape" or red highlight.
- 🔒 = a fact that also lives in `CONTEXT.md`. Change it here and it changes there too.
- Lengths matter where noted — some lines can't wrap without breaking the layout.

---

## 0 · Global

| ▸ Field | Current text |
| --- | --- |
| Browser tab title | Jonathan Fors — Ultra-Endurance Running Coach |
| Meta description | Ultra-endurance running coach, UESCA-certified. Holistic coaching that fits training around your life — from your first ultra to a hundred-miler. Book a free intro call. |
| Email | coach@jonathanfors.com |
| Instagram | @jonathans.pov |
| Booking link | meet.jonathanfors.com/discovery |

### Navigation bar

| ▸ Field | Current text |
| --- | --- |
| Logo alt text | Jonathan — Ultra Running Coach |
| Link 1 | Approach |
| Link 2 | How it works |
| Link 3 | Coaching |
| Link 4 | Newsletter |
| Button (red) | Book 1:1 Call |

---

## 1 · Hero

> ▸ **Eyebrow**
> Ultra-endurance running coach · UESCA-certified

> ▸ **Headline** — three lines, set at three different sizes. Line 3 sits on the red tape and **cannot wrap**, so keep it under ~16 characters.
> Train for
> the ultra.
> **Keep your life.**

> ▸ **Body**
> Holistic coaching for ambitious runners with jobs, families, and a long race on the horizon. The training is built to fit your life — not the other way around.

| ▸ Field | Current text |
| --- | --- |
| Button 1 (red) | Book 1:1 call |
| Button 2 (outline) | How it works |
| Photo caption plate | Coach · Est. 2026 |

### Stat rail (three plates at the foot of the hero)

| ▸ Figure | ▸ Label under it |
| --- | --- |
| UESCA | Certified ultrarunning coach |
| 744 km 🔒 | Portugal's coast in 10 days 🔒 |
| 1:1 | Remote coaching, worldwide |

### Red scrolling ticker

Six items, repeated. Keep them short — they scroll past.

1. 1:1 Ultra Coaching
2. UESCA Certified
3. First Ultra to 100 Miles
4. Holistic Health First
5. 744 km Across Portugal 🔒
6. Remote — Worldwide

---

## 2 · Section 01 — The approach

| ▸ Field | Current text |
| --- | --- |
| Section number | 01 |
| Section label | The approach |

> ▸ **Headline** — the tape sits on "your life." and cannot wrap.
> Training fits around **your life.** Not the other way around.

> ▸ **Black panel label**
> Holistic health

> ▸ **Black panel, paragraph 1**
> Most training plans assume your life will bend to the schedule. Mine assumes the opposite. We start with your work, your family, and the stress you're already carrying — then build the training to fit inside it.

> ▸ **Black panel, paragraph 2** (second sentence is bold on the page)
> The whole athlete comes first. **Consistency you can actually sustain beats a perfect week you can only manage once.**

### "Who this is for" — numbered list

| ▸ No. | Current text |
| --- | --- |
| 01 | Runners going from a first ultra up to a hundred-miler. |
| 02 | Ambitious adults with real jobs, families, and not enough hours. |
| 03 | People who distrust hype and want a coach who does the work. |

---

## 3 · Portugal band (photo, full width)

| ▸ Field | Current text |
| --- | --- |
| Eyebrow | Done, not planned |

> ▸ **Headline**
> I ran the coast of Portugal.

> ▸ **Paragraph 1** 🔒 (every number here is a real fact)
> Two years earlier the same route stopped me at 480 km. In 1–10 August 2026 I went back and finished it — 744 km in 10 days, for men's mental health, with Niki driving support.

> ▸ **Paragraph 2**
> That's the standard I coach from: not theory, but what actually holds up when the week falls apart.

### Result plates

| ▸ Figure | ▸ Label |
| --- | --- |
| 744 🔒 | Kilometres |
| 10 🔒 | Days on the road |
| 480 🔒 | Km, first attempt |
| 1–10 Aug 🔒 | 2026 |

---

## 4 · Instagram strip

| ▸ Field | Current text |
| --- | --- |
| Left label | As seen on Instagram |
| Figure | 11.1k |
| Figure label | Followers |
| Handle | @jonathans.pov |

---

## 5 · Section 02 — The program

| ▸ Field | Current text |
| --- | --- |
| Section number | 02 |
| Section label | The program |

> ▸ **Headline** — "rebuilt" is red.
> A plan built for your week — and **rebuilt** when it changes.

### The four steps

| ▸ No. | ▸ Title | ▸ Body |
| --- | --- | --- |
| 01 | The intro call | A free, no-pressure conversation about where you are and where you want to go. |
| 02 | Your full picture | A short intake covers your running history, your life, and your constraints — not just your splits. |
| 03 | A plan that fits | Structured training mapped onto your real schedule, with the load and recovery balanced for you. |
| 04 | Ongoing adjustment | The plan moves when life moves. We adapt as work, travel, and fatigue shift week to week. |

### Red callout at the end of the section

| ▸ Field | Current text |
| --- | --- |
| Heading | Programs are tailored, not off-the-shelf. |
| Body | What that looks like — and what it costs — depends on you and your race. We map it out together on the call. |
| Button | Book 1:1 call |

---

## 6 · Section 03 — Ways to work with me

| ▸ Field | Current text |
| --- | --- |
| Section number | 03 |
| Section label | Ways to work with me |

> ▸ **Headline** — the tape sits on "small group." and cannot wrap.
> Start with a **small group.**

**Layout:** group coaching leads as the big black block (it's the easier way in);
1:1 sits underneath as a slimmer outlined bar (the premium option).

### 6a · Group coaching — the main block

| ▸ Field | Current text |
| --- | --- |
| Top bar label | Group coaching |
| Status tag (red) | Coming soon |
| Price figure | Free |
| Price note | For the first 5 athletes 🔒 |
| Second price line 🔒 | Then €50/mo |

> ▸ **Body**
> Same structure and the same holistic approach as the 1:1 work, with people around you doing it at the same time. The easiest way in — and for the first five, it costs nothing.

**How the groups are built** — each row is a title plus a line of detail:

| ▸ Title | ▸ Detail |
| --- | --- |
| Kept small | Small enough that you're coached, not processed — and that everyone knows who you are. |
| Matched on purpose | Runners whose goals and constraints look like yours, so the conversation stays relevant. |
| Mixed on level | Close enough to relate, far enough apart to pull each other along. |

**Waitlist box** (right-hand side of the black block):

| ▸ Field | Current text |
| --- | --- |
| Big figure | 05 🔒 |
| Figure label | Free spots, first come |
| Heading | Join the waitlist |
| Note | The free spots go in the order people sign up. |
| Field placeholder | Enter your email |
| Button (red) | Notify me |
| Success message | You're in — check your inbox |

### 6b · 1:1 coaching — the bar underneath

| ▸ Field | Current text |
| --- | --- |
| Label | 1:1 Coaching |
| Price 🔒 | €200 |
| Price period | /mo |
| Button (outlined) | Book a 1:1 call |
| Note under button | Free · 30 minutes |

> ▸ **Body**
> The premium option. We set goals that fit your life and the direction you want it to go, then work at them together — week by week, adjusting as things change. First ultra or hundredth.

**What's included** (two columns, each with a red slash bullet):

1. 24/7 access to me on WhatsApp
2. A weekly 1:1 call
3. No template workouts — every session written for you
4. Extra calls whenever you need them
5. No minimum period. Cancel anytime

---

## 7 · Section 04 — Shuffle Club

| ▸ Field | Current text |
| --- | --- |
| Section number | 04 |
| Section label | The newsletter |

> ▸ **Headline** — "Free forever." is red.
> Three editions a week. **Free forever.**

> ▸ **Body**
> Every week takes one subject and works it from three sides — the research, my own read on it from the road, and whatever you ask. No fee, no upsell, no plans to change that.

### The week 🔒

| ▸ Day | ▸ Title | ▸ Description |
| --- | --- | --- |
| Tuesday | The science | What the research actually says. |
| Friday | My take | The same subject from the road, in my own words. |
| Sunday | Your questions | Reader questions, answered. |

| ▸ Field | Current text |
| --- | --- |
| Signup box heading | Subscribe |
| Signup box note | Tuesdays, Fridays and Sundays. Unsubscribe whenever you like. |
| Field placeholder | Enter your email |
| Button (red) | Subscribe |
| Success message | You're in — check your inbox |

The Shuffle Club logo above the headline is the dark-background version — red type,
with the slashes and "CLUB" in off-white.

---

## 8 · Closing call to action (red section)

| ▸ Field | Current text |
| --- | --- |
| Eyebrow | UESCA-certified |

> ▸ **Headline**
> Let's find your distance.

> ▸ **Body**
> Start with a free intro call. If it's a fit, we build from there. If it's not, you'll still leave with a clearer idea of your next step.

| ▸ Field | Current text |
| --- | --- |
| Button | Book a free 1:1 call |

---

## 9 · Footer

| ▸ Field | Current text |
| --- | --- |
| Blurb | Ultra-endurance running coach, UESCA-certified. Coaching built around the whole athlete. |
| Column 1 heading | Page |
| Column 2 heading | Start |
| Column 2 links | Book a 1:1 call · Email |
| Column 3 heading | Elsewhere |
| Column 3 links | Instagram · LinkedIn |
| Copyright | © 2026 Jonathan Fors. All rights reserved. |
| Right-hand line 🔒 | 744 km down the coast of Portugal for men's mental health. |

---

## 10 · Booking popup

Opens full-screen when any "Book a call" button is pressed.

| ▸ Field | Current text |
| --- | --- |
| Eyebrow | Free · 30 minutes |
| Heading | Book an intro call |
| Close button | Close |
| Loading text | Loading the calendar… |

The calendar itself is TidyCal — its wording is set in your TidyCal account.

---

## 11 · 404 page

Shown for any URL that doesn't exist — mistyped links, old addresses, anything
removed. Unlike the rest of this file it's **already live across the whole site**,
not just on `/redesign`.

| ▸ Field | Current text |
| --- | --- |
| Browser tab title | Page not found — Jonathan Fors |
| Top-right label | Error 404 |
| Eyebrow | Page not found |

> ▸ **Headline** — "off the map." is in red.
> You've run **off the map.**

> ▸ **Body**
> This page isn't here — it may have moved, or the link that brought you may be wrong. Nothing's broken. Head back and start from the top.

| ▸ Field | Current text |
| --- | --- |
| Button 1 (red) | Back to the homepage |
| Button 2 (outline) | Book 1:1 call |
| Footer copyright | © 2026 Jonathan Fors. All rights reserved. |
| Footer links | Email · Instagram |

The big outlined **404** is the section-index numeral treatment from the homepage,
scaled up — it's decorative and hidden from screen readers, so the "Page not found"
eyebrow is what actually gets read out.

Two deliberate omissions: no nav bar (its links are in-page anchors that would
land on nothing from a 404) and no newsletter signup (a dead end is the wrong
place to ask for an email).

---

## Notes

- **The signup fields are ours now**, not a beehiiv embed — so the placeholder,
  button label and confirmation line above are all editable copy. Submitting
  completes the signup on the page — no redirect — and swaps in the success
  message. The two forms are told apart in beehiiv by `utm_medium`:
  `group-coaching-waitlist` for the waitlist, `newsletter` for Shuffle Club.
  Verified working against the live beehiiv API on 18 Aug 2026. If beehiiv is
  ever unreachable the form quietly falls back to the old magic link instead of
  failing, so a signup is never lost.

- **Photos** — there are three, all from the recent shoot. Each has alt text, which is
  what screen readers and search engines see:

  | ▸ Where | ▸ Alt text |
  | --- | --- |
  | Hero | Jonathan Fors in a running top and visor, smiling, on a sunlit coastal trail. |
  | The approach | Jonathan Fors running up a sandy trail between low cliffs. |
  | Portugal band | Jonathan Fors running a sandy coastal trail. |

  Tell me if any of these describe the wrong thing.
- **Voice check** before you rewrite anything: direct, grounded, understated. No hype,
  no exclamation marks, no "unlock your potential".
- **Prices and dates** are held in one file (`lib/site.ts`) so they can never disagree
  between sections. Change them here and I'll update that one place.
